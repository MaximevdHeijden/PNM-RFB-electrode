...
Input of script
    ...
    4. A .CSV file containing experimental polarization curve data (current density - potential)
...
Output_folder = 'Local_km_OpenPNM'              # Output folder 
name0_vec = ['No_fitting']                      # Simulation name

# Setting simulation parameters
v_in_vec = [5e-2]                               # Inlet velocities [m/s]    
j0_vec = [773]                                  # Current densities [A/m2]  
...
for NrNetwork, file in enumerate(file_vec):
    for v_in in v_in_vec:
        ...
        def iterative_solver(params, anolyte, catholyte, net_c, net_a, E_cell_vec, rp_a, rp_c, j0, data, oc_c, oc_a,
                                                                                ad_c_even, ad_c_odd, ad_a_even, ad_a_odd, sf_c_odd, sf_c_even, sf_a_odd, sf_a_even, param, Velocity_dependent_km, Min_it_mass_charge):
            
            # Ascribing fitting values. Note that if you do *not* want to fit
            # a certain factor set this factor to unity.
            km_factor = 1.0                     # For vel-dep km: fit for pre-exponential factor of *Local* MT correlation     
            Conductivity_factor = params['Conductivityfactor'].value    # Conductivity fitting factor
            Rm_factor = 1.0                     # Membrane resistance factor
            SA_factor = 1.0                     # Relative Surface area (SA) factor
